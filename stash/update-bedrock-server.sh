...
# Find if Minecraft Bedrock exist on the file system. If multiple installations exist choose which one to update.
find_installations() {
    find /opt/ -type d -name "*.bak" -prune -o -type f -name "bedrock_server" -execdir test -e server.properties \; -printf "%h\n"
}
...
# Stop Docker container to safely check version
echo "Stopping Minecraft server..."
docker compose down

# Start server temporarily to get version info
...
if [[ $zip_file == *$version* ]]; then
    ...
    # Restart the server since we stopped it
...
cp -a $backup_dir/docker-compose.yml $installation_dir

# Set proper ownership (UID 1000 for Docker)
chown -R 1000:1000 $installation_dir

# Start updated server using Docker Compose
echo "Starting updated Minecraft server..."
docker compose up -d
...
echo "Update completed! Server should be running with the latest version."
echo "Use 'docker compose logs -f' to view server logs."
